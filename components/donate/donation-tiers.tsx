...
export const TIERS: Tier[] = [
  { id: 'coffee', emoji: '☕', name: 'Coffee', amountCents: 500, description: 'Buy the team a coffee' },
  { id: 'supporter', emoji: '🌱', name: 'Supporter', amountCents: 2000, description: 'Keep the servers running' },
  {
    id: 'sponsor',
    emoji: '🚀',
    name: 'Sponsor',
    ...
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
      {TIERS.map((tier) => (
        ...
        >
          ...
          <span className="mb-2 font-mono text-lg font-bold text-brand">${(tier.amountCents / 100).toFixed(0)}</span>
          <span className="text-xs text-white/45">{tier.description}</span>
          {tier.perks && (
            <span className="mt-2 text-[10px] text-brand-light/70">{tier.perks}</span>
          )}
        </button>
      ))}
    </div>
