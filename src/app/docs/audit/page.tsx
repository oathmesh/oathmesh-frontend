...
const fields = [
  ...
  { field: 'act', type: 'string[]', desc: 'Actions from the token act claim.' },
  {
    ...
    type: 'string',
    desc: 'The specific action required by the endpoint.',
  },
  ...
  { field: 'step', type: 'number (1–14)', desc: 'The verification step that failed. 0 on allow.' },
  {
    field: 'latency_ms',
    type: 'number',
    desc: 'Time in milliseconds from token receipt to decision.',
  },
...
export default function AuditPage() {
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <h1>Audit Logs</h1>
      <p>
        OathMesh writes a structured NDJSON log line for every token verification attempt — both
        allowed and denied. The logs contain enough information to reconstruct every
        machine-to-machine call in your infrastructure.
      </p>

      <h2>Log format</h2>
      <p>
        Each line is a valid JSON object. Lines are separated by newlines (NDJSON / JSON Lines
        format). This makes them compatible with all major log aggregators without additional
        parsing.
      </p>
      ...
      <Callout type="info" title="Replay deny events">
        When a token is rejected at step 12 (replay check), the <code>jti</code> of the replayed
        token is still logged. This lets you identify replay attacks by correlating multiple deny
        events with the same <code>jti</code>.
      </Callout>
