...
export const metadata: Metadata = {
  title: 'Go SDK',
  description: 'OathMesh Go SDK — chi middleware, context extraction, full example.',
};
...
export default function GoSdkPage() {
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <h1>Go SDK</h1>
      <p>
        The OathMesh Go SDK provides a chi middleware and a low-level verifier for direct
        integration.
      </p>
...
func main() {
	...
	http.ListenAndServe(":8080", r)
      ...
      <CodeBlock
        code={`caller := oathmeshchi.CallerFromContext(ctx)
if caller == nil {
...
fmt.Printf("caller: %s, actions: %v, token: %s\\n",
    caller.Subject, caller.Actions, caller.TokenID)`}
        language="go"
      />

      <Callout type="info" title="Scoped action check">
        The middleware verifies the token is structurally valid and belongs to the configured
        audience. To additionally require a specific action on a per-route basis, use{' '}
        <code>oathmeshchi.RequireAction("delete")</code> as a second middleware on the route.
      </Callout>

      <h2>Minting tokens (caller side)</h2>
      <CodeBlock
        code={`issuer := oathmesh.NewIssuerClient("https://issuer.internal")
