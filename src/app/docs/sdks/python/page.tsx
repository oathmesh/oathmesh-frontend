...
export const metadata: Metadata = {
  title: 'Python SDK',
  description: 'OathMesh Python SDK — FastAPI, Flask, Django integration examples.',
};
...
export default function PythonSdkPage() {
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <h1>Python SDK</h1>
      <p>
        The <code>oathmesh</code> PyPI package supports FastAPI, Flask, and Django out of the box.
      </p>
...
async def get_invoice(
    ...
    return {"id": id, "caller": caller.subject}`}
        language="python"
        filename="main.py"
      />
...
def list_invoices():
    ...
    return jsonify({"invoices": [], "caller": caller.subject})`}
        language="python"
        filename="app.py"
      />
...
def list_invoices(request):
    ...
    return JsonResponse({"invoices": [], "caller": caller.subject})`}
        language="python"
      />

      <h2>Minting tokens</h2>
      <CodeBlock
        code={`from oathmesh import mint_token
import httpx
