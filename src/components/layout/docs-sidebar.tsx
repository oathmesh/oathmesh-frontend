...
function NavSection({ item }: { item: NavItem }) {
  ...
  return (
    <div>
      <button
        onClick={() => setOpen((o) => !o)}
        className="flex w-full items-center justify-between rounded-md px-3 py-1.5 text-xs font-semibold uppercase tracking-wider text-white/40 hover:text-white/50 transition-colors"
        aria-expanded={open}
      >
        {item.label}
