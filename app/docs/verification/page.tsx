...
export const metadata: Metadata = {
  ...
  description:
    'Full walkthrough of OathMesh\'s 14-step fail-closed token verification pipeline.',
};
...
export default function VerificationPage() {
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <p>
        Every OathMesh receiver runs an identical 14-step verification pipeline
        before passing the request to application logic. The pipeline is{' '}
        <strong>fail-closed</strong>: any step failure immediately returns an
        error response. There is no partial pass.
      </p>

      <Callout type="info" title="Clock skew">
        Steps 7 and 8 allow ±10 seconds of clock skew between the issuer and
        receiver clocks. This is intentionally conservative. If your
        infrastructure has larger clock skew, fix NTP rather than increasing
        this tolerance.
      </Callout>
          ...
          >
            <div className="flex items-start gap-4 p-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="mb-0.5 font-semibold text-white text-sm">
                  {step.name}
                </p>
      ...
      <p>
        OathMesh allows ±10 seconds of clock skew on expiry and not-before
        checks. The allowance exists to handle minor NTP drift between the
        issuer and receiver hosts. It does not materially extend the token
        lifetime — a 300-second token with 10 seconds of leniency is still a
        310-second worst-case window, not infinite.
      </p>

      <h2>Replay cache implementation</h2>
      <p>
        The replay cache stores seen <code>jti</code> values until their
        corresponding token would have expired (<code>exp + 10s</code>). By
        default, OathMesh uses an in-memory cache (fast, no external
        dependency) that is not shared between instances. For multi-instance
        deployments, configure Redis as the replay cache backend:
      </p>
      <p>
        See the <a href="/docs/faq">FAQ</a> for a comparison of Redis vs
        in-memory replay caching trade-offs.
      </p>

      <h2>Audit log on completion</h2>
      <p>
        After all 14 steps pass (or fail), the verifier writes a single NDJSON
        audit line with the result, latency, and all relevant claims. See{' '}
        <a href="/docs/audit">Audit Logs</a> for the full field spec.
      </p>
    </article>
  );
}
