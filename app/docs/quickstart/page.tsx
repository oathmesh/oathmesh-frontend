...
export default function QuickstartPage() {
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <h1>Quick Start</h1>
      <p>
        This guide takes you from zero to a working OathMesh setup in under 10
        minutes. You will install the CLI, generate an Ed25519 key pair, start
        the issuer, mint a token, call a protected endpoint, and add the
        middleware to your own service.
      </p>

      <Callout type="info" title="Prerequisites">
        Docker ≥ 20.10, Node.js ≥ 18, and the OathMesh CLI. The CLI can be
        installed via npm, Homebrew, or downloading a binary from GitHub
        Releases.
      </Callout>
...
brew install oathmesh`}
          ...
          },
          {
            ...
            children: (
              <>
                <p>
                  OathMesh uses Ed25519 exclusively. Generate a key pair and store
                  it securely — the private key is never transmitted.
                </p>
                <CodeBlock
...
# ✓ Public key  written to ./keys/oathmesh.pub  (share with issuer)`}
                ...
                <Callout type="warning" title="Protect your private key">
                  Never commit <code>oathmesh.key</code> to source control. Add{' '}
                  <code>*.key</code> to your <code>.gitignore</code>. In
                  production, store it in a secret manager (AWS Secrets Manager,
          ...
          },
          {
            ...
            children: (
              <>
                <p>
                  The OathMesh issuer is a small HTTP server that accepts mint
                  requests, validates identity, applies policy, and returns signed
                  tokens.
                </p>
                <CodeBlock
                  code={`docker run -d \\
  ...
  ghcr.io/oathmesh/issuer:latest`}
                ...
                />
                <p>
                  The issuer is now listening on <code>http://localhost:4000</code>
                  . It serves a JWKS endpoint at <code>/jwks.json</code> that
                  receivers use to fetch the public key.
                </p>
              </>
            ),
          },
          {
            ...
            children: (
              <>
                <p>
                  Mint a token for the <code>payments-svc</code> service to call
                  the <code>billing-api</code>:
                </p>
                <CodeBlock
                  code={`oathmesh mint \\
  ...
  http://localhost:8080/invoices`}
          ...
          },
          {
            ...
            children: (
              <>
                <p>
                  Install the SDK for your platform and add the middleware. Here is
                  the Express example — see the{' '}
                  <a href="/docs/sdks">SDK reference</a> for Go and Python.
                </p>
                <CodeBlock
                  code={`npm install oathmesh`}
                  language="bash"
                />
                <CodeBlock
...
app.listen(8080, () => console.log('listening on :8080'));`}
                ...
                <Callout type="info" title="What the middleware does">
                  The middleware fetches the JWKS from the issuer (cached), verifies
                  the Ed25519 signature, checks expiry + clock skew (±10s), validates
                  the jti against a replay cache, checks audience and actions against
                  the active policy, and writes an audit entry. Any failure returns 401.
                </Callout>
              </>
            ),
          },
      ...
      <ul>
        <li>
          <a href="/docs/token-format">Token format</a> — understand every claim in
          an OathMesh token
        </li>
        <li>
          <a href="/docs/verification">Verification pipeline</a> — the 14 steps
          your receiver runs
        </li>
        <li>
          <a href="/docs/policy">Policy engine</a> — write Pkl policies to
