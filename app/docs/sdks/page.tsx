...
export default function SdksPage() {
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <h1>SDK Overview</h1>
      <p>
        OathMesh provides official SDKs for the three most common backend
        ecosystems. Each SDK implements the full 14-step verification pipeline and
        writes to the NDJSON audit log.
      </p>
          ...
          >
            <div className="flex-1">
              ...
              <p className="text-xs text-white/45">
                Frameworks: {sdk.frameworks.join(', ')}
              </p>
            </div>
            <ArrowRight className="h-4 w-4 shrink-0 text-white/30 mt-1" />
          </Link>
        ))}
      </div>
      ...
      <p>
        The following SDKs are maintained by the community and are not officially
        supported by the OathMesh team:
      </p>
      <ul>
        <li>
        ...
        <li>
          <strong>Java/Spring Boot</strong> — <code>oathmesh-spring</code>{' '}
          (planned, see <Link href="/wishlist">wishlist</Link>)
        </li>
      </ul>
    </article>
  );
