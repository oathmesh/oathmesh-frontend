  ...
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <p>
        ...
        as its policy language. Pkl is a type-safe configuration language that
        compiles to JSON/YAML. OathMesh evaluates Pkl policies at verification
        time and hot-reloads them without restarting the issuer or receivers.
      </p>

      <h2>Why Pkl?</h2>
      <p>
        Unlike OPA Rego or CEL, Pkl policies are statically typed and readable
        by engineers who have never used a policy language before. A Pkl policy
        reads like configuration, not code. The type system catches errors at
        write time, and the hot-reload path makes zero-downtime policy updates
        practical.
      </p>

      <h2>Default-deny semantics</h2>
      <p>
        OathMesh policies are default-deny. If no rule explicitly allows a (sub,
        aud, act) combination, the request is denied. This means the safe
        failure mode is always denial, not permission. You write allow rules;
        everything else is denied automatically.
      </p>

      <Callout type="warning" title="Empty policy = total deny">
        If the policy file is empty or fails to parse, OathMesh falls back to
        denying all tokens. It does not fall back to allowing all — that would
        defeat the purpose of having a policy.
      </Callout>
...
rules: List<Policy.Rule> = new {
  ...
  new Policy.Deny {
    ...
    reason = "audit-reader is read-only by policy"
      ...
      <h2>Hot-reload</h2>
      <p>
        The issuer watches the policy file for changes using filesystem events
        (inotify on Linux, FSEvents on macOS). When a change is detected:
      </p>
      <ol>
        <li>The new policy is compiled and type-checked in a sandbox</li>
        <li>
          If compilation fails, the existing policy remains active and an error
          is logged — the issuer never switches to an invalid policy
        </li>
        <li>
          If compilation succeeds, the new policy atomically replaces the old
          one — the swap is done under a read-write lock so in-flight
          verifications complete under the old policy
        </li>
      </ol>
      <p>
        To trigger a manual reload without a file change (e.g., after a
        ConfigMap update in Kubernetes):
      </p>
      <CodeBlock
...
curl -X POST http://localhost:4001/admin/reload-policy`}
      ...
      <p>
        Use <code>subjectPrefix</code> to allow all services in a team to access
        a shared API:
      </p>
      <CodeBlock
