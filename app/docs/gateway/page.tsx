...
export const metadata: Metadata = {
  title: 'Gateway Mode',
  description:
    'Use OathMesh as a reverse proxy to protect existing services without SDK changes.',
};
...
export default function GatewayPage() {
  return (
    <article className="prose-oathmesh mx-auto max-w-3xl px-6 py-10">
      ...
      <h1>Gateway Mode</h1>
      <p>
        Gateway mode runs OathMesh as a reverse proxy in front of an existing
        HTTP service. It verifies incoming OathMesh tokens, strips the
        Authorization header, injects verified identity headers, and forwards the
        request. The upstream service receives the request only after verification
        passes — it never sees invalid tokens.
      </p>

      <Callout type="tip" title="No SDK required">
        Gateway mode lets you add OathMesh protection to any HTTP service without
        modifying its source code. The upstream service reads caller identity from
        injected headers instead of parsing a JWT.
      </Callout>
...
oathmesh serve \\
  ...
  --issuer https://issuer.internal`}
      ...
      <p>
        When a token passes verification, the gateway strips the original
        Authorization header and injects these headers before forwarding:
      </p>
      ...
      <Callout type="warning" title="Trust only from the gateway">
        Your upstream service must only accept these headers from the gateway
        itself — not from external callers. Add a network policy or firewall rule
        so the upstream port is not publicly reachable. Alternatively, configure
        the gateway to add a shared secret header that the upstream validates.
      </Callout>
